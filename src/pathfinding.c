...
/*   Updated: 2023/11/30 15:17:52 by gbricot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (data->map[tab])
	{
		...
		while (data->map[tab][i])
		{
				...
				|| data->map[tab][i] == 'E' || data->map[tab][i] == 'W')
			{
				if (!data->player)
				{
					data->player = ft_calloc(sizeof(t_coords), 1);
					data->player->x = i;
					data->player->y = tab;
				}
				else
...
{
	//usleep(10000);
	//printf("Run Through : y=%d, x=%d | limits :x%d, y%d\n", y, x, data->map_max_y, data->map_max_x);
	if (map[y][x] == '1')
		return ;
	if (x <= 0 || y <= 0 || x >= data->map_max_x - 1 || y >= data->map_max_y - 1)
	{
		...
		{	
			data->err = 1;
			//printf("err with y=%d, x=%d\n", y, x);
			return ;
		}
		//printf("On the border return\n"); //useless
		return ;
	}
	if (map[y][x] == ' ')
	{
...
char	ft_pathfinding(t_data *data)
{
	...
	ft_get_player_coords(data);
	 if (data->player)
	 	printf("[DEBUG] player coords : y = %d | x = %d\n", data->player->y, data->player->x);
	if (!data->player)
		return (0);
	i = 0;
	...
	cpy_map[i] = NULL;
	ft_run_through(data, cpy_map, data->player->y, data->player->x);
	i = 0;
	while (cpy_map[i])
		free (cpy_map[i++]);
	free (cpy_map);
